...
create function private.require_transactional_save() returns trigger
language plpgsql security invoker set search_path='' as $$
begin
 if auth.uid() is not null and coalesce(auth.role(),'')<>'service_role' and coalesce(current_setting('crm.transactional_save',true),'')<>'1' then
  raise exception 'Esta pestaña usa una versión anterior del CRM. Conserva la captura y actualiza la página antes de guardar.' using errcode='55000';
 end if;
 if tg_op='DELETE' then return old;else return new;end if;
end $$;
revoke all on function private.require_transactional_save() from public;
do $$declare t text;begin
 foreach t in array array['clients','leads','agenda_events','collaborators','message_templates','services','app_settings'] loop
  execute format('create trigger require_transactional_save before insert or update or delete on public.%I for each row execute function private.require_transactional_save()',t);
 end loop;
end $$;
commit;
