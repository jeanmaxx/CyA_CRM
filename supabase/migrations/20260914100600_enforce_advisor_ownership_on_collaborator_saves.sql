create or replace function public.crm_save_changes(operations jsonb) returns jsonb
language plpgsql set search_path='' as $$
declare
 op jsonb; t text; rid text; incoming jsonb; previous jsonb; saved jsonb;
 org uuid:=public.current_org_id(); cols text; vals text; sets text; keycol text;
 result jsonb:='[]'; app_role text:=public.current_app_role();
begin
 ...
 perform set_config('crm.transactional_save','1',true);
 for op in
  select value from jsonb_array_elements(operations)
  order by (value->'row'='null'::jsonb), array_position(array['collaborators','leads','clients','agenda_events','message_templates','services','app_settings'],value->>'table'),value->>'id'
 loop
  ...
  if t in ('services','app_settings') and app_role<>'tech_admin' then raise exception 'Acceso técnico requerido'; end if;
  keycol:=case when t='app_settings' then 'organization_id' else 'id' end;
  if not pg_try_advisory_xact_lock(hashtextextended(org::text||t||rid,0)) then
   raise exception 'Registro ocupado: otro guardado está procesando %. Intenta nuevamente.',rid using errcode='40001';
  end if;
  ...
  if t='collaborators' and incoming is not null and app_role='advisor' then
   incoming:=jsonb_set(incoming,'{advisor_id}',to_jsonb(auth.uid()),true);
   incoming:=jsonb_set(incoming,'{legacy_advisor_id}','null'::jsonb,true);
   if jsonb_typeof(incoming->'payload')='object' then
    incoming:=jsonb_set(incoming,'{payload,asesorId}',to_jsonb(auth.uid()::text),true);
   end if;
  end if;
  ...
  select string_agg(format('%I',key),','),string_agg(format('x.%I',key),','),string_agg(format('%I=x.%I',key,key),',')
    into cols,vals,sets from jsonb_object_keys(incoming) key;
