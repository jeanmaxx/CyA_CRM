...
assert.equal(d.querySelectorAll('#ct-visor section.docx').length,4);assert.equal(d.querySelectorAll('#ct-visor footer').length,4);assert([...d.querySelectorAll('#ct-visor footer')].every(f=>!f.textContent.trim()));
...
assert.deepEqual(errors,[]);console.log('PASS: Synthetic Word rendering, header and empty footer, immutable fields/history versions, auto save and invalidation after editing.');clearTimeout(deadline);dom.window.close();
