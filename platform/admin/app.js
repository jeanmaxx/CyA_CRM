...
function applyRoleUI(){const role=state.admin?.role||'';$$('[data-role-view="owner"]').forEach(el=>el.hidden=role!=='owner');$$('[data-role-view="billing"]').forEach(el=>el.hidden=!['owner','admin','billing'].includes(role));if($('#new-client'))$('#new-client').hidden=!['owner','admin'].includes(role)}
...
$('#new-client')?.addEventListener('click',()=>{$('#client-form').reset();$('#org-slug').dataset.touched='';fillPlanOptions();$('#client-dialog').showModal()});
